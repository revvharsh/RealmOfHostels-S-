...
export const AnnouncementsTab: React.FC<AnnouncementsTabProps> = ({ darkMode, notices }) => {
  ...
  const announcements = [
    ...notices.map((notice) => ({
      id: notice.id,
      title: notice.title,
      category: notice.target || 'Hostel Notice',
      date: notice.date,
      priority: notice.priority === 'High' ? 'Urgent' : notice.priority,
      content: notice.body,
      acknowledged: acknowledgedIds.includes(notice.id),
    })),
    ...ANNOUNCEMENTS_LIST.filter((item) => !notices.some((notice) => notice.id === item.id)).map(
      (item) => ({
        ...item,
        acknowledged: item.acknowledged || acknowledgedIds.includes(item.id),
      })
    ),
  ];
